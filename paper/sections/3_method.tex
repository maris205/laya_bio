\section{Laya-Bio: a shared biological decision interface}
\paragraph{Input and output contract.}
The interface is $(s,q,C)\mapsto(p,\hat\jmath)$: a sequence, a question, and a nonempty list of uniquely identified candidate labels yield probabilities and a selected candidate index. For finite logits, $\hat\jmath=\arg\max_{1\leq j\leq K}p_j$, and the application returns the supplied label $c_{\hat\jmath}$ through an exact lookup. Consequently $c_{\hat\jmath}\in C$ by construction. This removes output-label invention and answer-string parsing from the decision path; it does not prove that $c_{\hat\jmath}$ is correct, that the candidates cover the true answer, or that their semantics are understood. Empty lists, nonfinite scores, or ambiguous label identifiers should fail validation rather than silently produce a decision.

One scorer is shared across tasks and candidate counts. New candidate strings do not require a new output matrix, subject to the input-length budget, but useful prediction on new labels remains an empirical transfer question. The experiments below use only the two trained task schemas. We evaluate the \emph{choice} interface; Score, Noul panels for multilabel prediction, and paired-sequence extensions are specified prospectively in Appendix~\ref{sec:prospective}, not evaluated here.
...
\paragraph{Secondary input-representation comparison.}
