\section{Initial results: accuracy and decision reliability}
\begin{table}[t]
\centering\small
...
\label{tab:main}
\end{table}
\paragraph{One candidate model across two biological tasks.}
Each candidate checkpoint jointly serves both tasks with the same output scorer. Raw-input candidate scoring reaches 91.10\% DNA and 59.97\% protein accuracy (Table~\ref{tab:main}). This establishes supervised reuse across the two trained schemas, rather than transfer to an unseen question. Output-set membership follows from candidate-index selection; the fixed-head control also returns a valid class. Valid output alone therefore cannot explain a predictive advantage or distinguish the two systems.
...
\paragraph{Secondary representation result.}
