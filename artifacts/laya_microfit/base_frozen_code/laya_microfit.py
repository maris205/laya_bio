...
def main():
    ...
    model,cfg=exp.build(a.kind,a.model_dir,a.laya_repo,info,device)
    ...
    for step in range(1,a.updates+1):
        model.train();chunk=training.copy();rng.shuffle(chunk);items=[i for e in chunk for i in e['items']]
        ...
        if step in {1,8,16,32,64,96,a.updates}:evaluate_step(step)
    ...
    summary={**config,'complete':True,'actual_updates':len(trace),'entity_presentations':32*len(trace),
        ...
        'initial':history[0],'final':final,'training_panel_fit_passed':fit_passed(final['training_panel']),
        'canonical_panel_fit_passed':fit_passed(final['canonical_panel']),
