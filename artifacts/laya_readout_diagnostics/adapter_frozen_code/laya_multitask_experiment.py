...
def pool_hidden(hidden, attention_mask, pooling):
    if pooling=='cls':return hidden[:,0]
    if pooling!='mean':raise ValueError('Unknown pooling: '+pooling)
    mask=attention_mask.to(hidden.dtype).unsqueeze(-1)
    return (hidden*mask).sum(1)/mask.sum(1).clamp_min(1)


def score_loss_values(logits, scalar, labels, targets, objective):
    ce=nn.functional.cross_entropy(logits,labels,reduction='none')
    mse=(scalar-targets).square()
    if objective=='joint':return .5*ce+.5*mse
    if objective=='ce':return ce
    if objective=='mse':return mse
    raise ValueError('Unknown Score objective: '+objective)
...
class SharedHeads(nn.Module):
    ...
    def forward(self,input_ids,attention_mask,qtype,task):
        ...
        h=self.readout(pool_hidden(h,attention_mask,self.pooling));scalar=self.regressors[task](h).squeeze(-1).float() if task in self.regressors else None
        ...
        if kind=='candidate':
            ...
            if primitive=='multilabel':
                ...
                if primitive=='score':
                    ...
                    loss=score_loss_values(logits,scalar,torch.tensor([i['label'] for i in items],device=device),target,model.score_loss)
...
def training_schedule(entities, seed, updates, effective_batch):
    """Replay the joint sampler exactly; a single-task run skips other updates."""
    rng=random.Random(seed);by_task={t:[e for e in entities if e['task']==t] for t in data.TASKS}
    for pool in by_task.values():rng.shuffle(pool)
    cursors=Counter();cycle=[]
    for step in range(1,updates+1):
        if not cycle:cycle=data.TASKS.copy();rng.shuffle(cycle)
        task=cycle.pop();pool=by_task[task];chunk=[]
        while len(chunk)<effective_batch:
            if cursors[task]>=len(pool):rng.shuffle(pool);cursors[task]=0
            chunk.append(pool[cursors[task]]);cursors[task]+=1
        yield step,task,chunk
...
def main():
    ...
    for task in task_info:
        ...
        diagnostic_rows.extend([r for r in rows['train'] if r['task']==task][:a.train_diagnostics_cap])
    train_diagnostic=make_items(diagnostic_rows,tok,builder,a.kind,a.seed,False,manifest['max_length'])
    ...
    for step,task,chunk in training_schedule(entities['train'],a.seed,a.updates,a.effective_batch):
        ...
        rec={'step':step,'optimizer_step':optimizer_step,'task':task,'lr':a.lr*factor,
             'entity_batch_sha256':data.sha('\n'.join(e['id'] for e in chunk)),'loss':step_loss,'gradient_norm':norm,'seconds':time.monotonic()-started}
        trace.append(rec)
    ...
    assert optimizer_step==config['updates']
    ...
    train_records,_=evaluate(model,a.kind,train_diagnostic,tok,device,task_info,a.eval_batch) if train_diagnostic else ([],0.)
    if train_records:(a.output/'train_diagnostic_predictions.jsonl').write_text(''.join(json.dumps(r)+'\n' for r in train_records))
    ...
    summary={**config,'complete':True,'training_seconds':training_seconds,'peak_allocated_gib':peak,
             ...
             'before':metrics(before,task_info),'after':metrics(after,task_info),'train_diagnostic':metrics(train_records,task_info),'dev_eval_seconds':after_seconds,
