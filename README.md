...
**Active route (2026-09-25):** One model, multiple biological tasks, unified JEV-style outputs, with task quality and inference cost as the decision criteria. The corrected vocabulary/CPT/fixed-classifier round is complete ([report](research/laya_biocpt_round1.md)): promoter CPT/no-CPT accuracy is 87.55%/82.51% at 4K labels and 89.92%/90.02% at full data. The new bounded round restores the shared typed candidate scorer and jointly trains promoter Noul, protein-structure Choice and GFP Score, with 8,192 training examples per task, matched no-CPT joint training and three single-task references. Data admission and train-only GPU checks passed; production is running. Few-shot/zero-shot are deferred; a Qwen biological adaptation route is the next backbone candidate if this round reveals material quality failures. See the [active plan](refine-logs/EXPERIMENT_PLAN.md). The old GFP microfit extension remains paused.
