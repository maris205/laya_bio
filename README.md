...
**Active route (2026-09-24):** The user paused the direct-SFT/GFP diagnostic route. The active work is fresh biological vocabulary → embedding/MLM-head adaptation → biological CPT → matched no-CPT/CPT single-task SFT using thousands of examples, followed by cumulative multitask evaluation. See the [active plan](refine-logs/EXPERIMENT_PLAN.md) and [pause record](refine-logs/ROUTE_CHANGE_20260924.md). The initial protein source failed a canonical-residue composition audit; its 4K result is retained as a superseded diagnostic. The corrected-source round is complete: CPT/no-CPT development accuracy is 87.55%/82.51% at 4,096 labels (+5.04 pp), and 89.92%/90.02% at 16,766 labels (−0.10 pp; no clear full-data accuracy gain). All five fits and independent audits pass. See the [source-quality finding](research/laya_protein_source_quality.md). See the [first-round report](research/laya_biocpt_round1.md).
