...
还不能确定唯一根因，也不能认定某种结构从原理上无法学习。只替换 readout 的曲线在最后阶段仍在降低误差，下一项应是对该配置做**只增加更新数的收敛检查**，保留样本、初始化、学习率和损失定义；例如从 128 延长到 512 步，核对前 128 步轨迹一致。该建议未在本轮执行，也不代表已证明更多训练一定奏效。通过训练拟合检查后，再回到 1,024 训练样本及独立开发集验证，之后才扩展任务或做正式架构比较。
